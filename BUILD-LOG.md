...
**Correction — step 1.17 was built on a misreading.** 1.17 says "every real report colours each finding and prints a legend" and BUILD-LOG says severity was "seeded by majority vote from the 17 real reports". Both are wrong. **Exactly one report colours its findings: Oh Dental.** The other sixteen contain no severity colour anywhere — the only `w:color` values in them are table fills and hyperlink blue — and the legend appears in **1 of 17**. The severities in the bank came from Oh Dental's 36 coloured bullets, which is a defensible source, but it is one reviewer's choice on one report, not house style. The feature stays because it was asked for by name; it is now recorded as a deliberate divergence and is **not scored as a gap**, so the ledger cannot reward matching a habit sixteen reports do not have. §13.2 1.17 is struck through and corrected in place.

**First reading — score 17, eight gaps.** Three major: star cells print three glyphs where 16 of 17 references always print five with the unearned ones tinted pale; the Comments column still falls back to the template's dimension list for the 6 of 9 categories the summariser had no findings for (1.18 fixed the mechanism, not the fallback — `N/A` is what the references print); and severity is painted on the finding *text* when Oh Dental paints the *bullet* and leaves the text black. Five minor: dd/mm/yyyy dates, and four thin sections — Website (Business) carries 1 finding against a reference median of 10.

**Bank coverage is better than previously reported.** 585 body paragraphs across the 17 reduce to 300 distinct ones; 47 recur in three or more reports — the house copy — and the bank already says **46**. The one miss is the SEO ranking paragraph, which is blocked on a SERP provider. The other 240 clusters appear in exactly one report: prose written for one practice, correctly absent from a bank of house paragraphs.

**Evidence:** 11 new tests, no database and no reference files — the reader is pinned by round-tripping documents the suite builds itself, because `Downloads/presence` exists on one machine and a test that depends on it is a test that fails on the other. Two reader bugs are pinned as regressions because both were found by hand and both would have failed *silently*: severity read as absent (the colour is on `w:pPr/w:rPr`, not the runs) and the footer page number read as absent (`<w:instrText>PAGE` has no whitespace around it). A third — `word/media/` counted as an image, so a report with no exhibits looked like it had one — is why the exhibit check now counts what the body *references* rather than parts in the archive. Full server suite 233/233, typecheck clean.

**Files:** `server/src/review/{docx-read,fidelity,fidelity-run}.ts`, `server/test/fidelity.test.ts`, `docs/FIDELITY-LEDGER.md` (new) · `server/package.json` (jszip as an explicit devDependency, `review:fidelity` script), `.gitignore`, `MASTER-BUILD-PLAN.md` (edited).

**Decisions:** (1) Three of 17 is the floor for calling something house style — below that it is one client's prose and copying it would overfit. That rule is what caught 1.17. (2) The blank template is excluded from the reference set: it is what a report is made *from*, and letting its unfilled dimension lists vote on house style is backwards. (3) The harness audits the same practice as the canonical reference. (4) It runs against `MockModelClient` by default (`--live` for the real one), so the score moves when the *document* changes rather than when the model has a different day. (5) A missing section is no longer also counted as thin — the same defect twice inflates the score and makes closing one gap look like closing two.
