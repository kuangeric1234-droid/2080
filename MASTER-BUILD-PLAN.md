...
- [ ] 1.32 **Positive halves of measured pairs** — the bank carries the negative half of several machine-measured checks and no positive half, so a site that passes gets silence and a site that fails gets a paragraph. Oh Dental's own report carries the positive for third-party links opening in a new tab, for the nav bar floating down, for navigation legible to the colour-impaired, and for a fast load. Count each across the 17 first; build the halves that clear the threshold, paired to the same signal as their negative twin (`render.external_links_same_tab`, `render.nav_sticky`, `render.contrast_violations`, `render.body_font_px`, `site.load_seconds`, `site.wp_admin_default`). **DoD:** for each pair, a passing site prints the positive and a failing site the negative, both halves can never ship together, and the pair test in `bank.test.ts` pins them by id.

- [ ] 1.33 **The three findings the reference contradicts** — the 05/08 export of ohdental.com.au says the site uses the default `/wp-admin` URL, that third-party links open in the same tab, and that the nav bar does not float. The 31/07 report on the same site says the opposite of all three. Five days apart, so one of the two is wrong about the facts. Re-run the probes against ohdental.com.au, determine which side is right, and fix whichever is broken — the probe, or the reference's claim. **DoD:** each of the three checks has a test built from the real markup or computed style it disagreed on, and the export's claim matches what the site actually does. If a probe was right and the human wrong, record that — it is worth knowing.

- [ ] 1.34 **Hosting and performance never reached the document** — `tech.hosting.speed` fires on `site.load_seconds` and 1.16 built the PageSpeed provider, yet the 05/08 export carries no hosting or speed paragraph and no `performance_report` exhibit, while the Technical section of 17 of 17 references carries the speed test. Diagnose why the signal is absent on this run before changing anything. Then add the positive half under 1.32's rule and fill the hosting-country and host-name variables the engine already knows about. **DoD:** a fast site prints the fast paragraph and a slow one the slow paragraph, the performance exhibit lands under Website (Technical), and a missing key produces nothing rather than a guess.

- [ ] 1.35 **Two sections that unconditional house copy would fill** — Visibility (SEM) and Social Media export empty. The reference's whole SEM section is one line (`sem.access.request`) and its Social section closes with `social.video.opportunity`; both sit in the bank as `when: "manual"`. Count both across the 17, and if they clear the threshold move them to `when: "always"`, setting `auto_safe` by the 1.8 rule with the reasoning recorded. **DoD:** neither section is empty on an unattended run, the draft notice names one fewer section for each, and neither line makes a claim about the practice.

- [ ] 1.36 **The summary is written once and goes stale** — the summariser runs at collect time, so anything a reviewer accepts afterwards never reaches it. That is why the 05/08 export prints `N/A` in the Comments cell for Website (Business) in a report whose Business section carries two bullets, and why the Recommendations paragraph describes a thinner report than the one it opens. Re-run it against the accepted set before export, keeping the grounding validator in the path and the refuse-rather-than-repair behaviour. **DoD:** accepting a finding and exporting produces a Comments cell and an opening paragraph that both reflect it; no category with accepted findings prints `N/A`; an ungrounded run still refuses and the reviewer sees the refusal.

- [ ] 1.37 **Competitors arrive as names and review counts only** — Places seeds the top three (1.31) but only `review_count` and `review_rating` are written, so `comp.row` drops fourteen of its eighteen fragments and each competitor is one short clause. `collectCompetitorFacts` already measures https, booking, platform and content volume and is only ever called on the manual path; call it on the seeded three too, and add opening hours to the Place Details fields to fill `days_open`. SERP, map and every `fb_` fragment stay absent — `NoSerpProvider` returns nothing on purpose and there is no social data. **DoD:** each row carries the facts we genuinely measured in `comp.row`'s own order; a competitor whose site cannot be reached still renders its name and review count rather than failing the export; nothing is asserted about a named third party that no signal supports.

- [ ] 1.38 **The Competition section has no closing verdict** — all three references end the section with one of `comp.verdict.open_field` / `leading` / `behind`, and all three are manual. 1.31 kept them manual for a stated reason: their own text says "judging by their website, social media and online reputation" and we have no social data, so firing them would assert something never looked at. Either fire the verdict only when every fact its sentence names is present, or find the variant in the 17 that claims only what we measure. The house copy is not to be edited to fit what we can measure. **DoD:** the section either ends in a verdict every clause of which traces to a signal, or it is documented why it cannot yet.
