...
- [x] 1.39 **The exam before the skill — and the first way of building it was wrong** — Website (Business) produces 2 findings against a reference average of 10, and all nine of its `judgement` paragraphs are calls a model could make from the screenshot and crawled copy we already hold. Before writing that skill, build its exam: for each of the 17 completed reports, did the human make each of those nine calls? **The first attempt matched by trigram overlap and was wrong.** About a third of every report is rewritten in the writer's own words — Paladin is 23% house voice — so it scored Paladin 0 of 9 when the report plainly makes at least five. Worse, the bank's positive/negative twins share an opening sentence, so string overlap of **0.88** matched "Each of the doctors *needs* their own biography page" to "Each of the doctors *has* their own biography page which is good" — the exact opposite finding. Replaced with `review-judge-labeller` (G0, labelling only, never reaches a client), which reads each report and must **quote the paragraph** behind every `true`; a quote not found in the report is discarded rather than trusted. **DoD:** 153 labels with an audit trail, zero unverifiable quotes, and the disagreements against the string method read by hand before the exam is used. *(Verified: of the 9 cases where string said yes and the model said no, 6 are the model correctly catching an inverted twin and 3 are defensible calls on a related-but-distinct finding. 0 quotes were rejected. Six of the nine snippets have four or more in the minority class and are worth scoring; `uvp.generic` 14/3, `conditions_content` 14/3 and `team_photos` 2/15 are too lopsided for accuracy to mean anything and must be scored per class.)*

- [x] 1.40 **The other half of the exam — what the model is shown** — the 1.39 labels say what the reviewer concluded about a site as it stood on the report's date, and 10 of the 17 reports are from 2024 or earlier. Scoring against today's homepage would measure how much the practice has changed, and would mark the model wrong for agreeing with the reviewer wherever the site has since been rebuilt — which is the very thing those reports recommended. Each case is now reconstructed from the Wayback capture nearest its report date via the `id_` raw endpoint (no replay toolbar in the text), with page types read by `classifyPaths` — **the same function the live collector uses**, so the exam measures the model and not a second classifier. Source is chosen **per case**: archive wins 14 of 16, five of them inside a fortnight; live wins for Oh Dental, reported six days ago against a 513-day-old capture. **DoD:** every case carries its source, its drift in days and a `replayUrl` a human can open to see exactly what the model saw. *(**16 of 17 scorable, 135 of 153 cells.** Advanced Chiropractic is excluded at 314 days drift; three sites render their navigation in JavaScript so no links parse, and for those the three page-count calls are marked unscorable rather than counted — zero parsed links is indistinguishable from a site with genuinely no bio pages, and treating it as the latter would mark a correct model wrong.)*

- [x] 1.41 **review-judge sat the exam and failed it** — the skill that was to close the Website (Business) gap scores **59% against a 73% majority baseline** over 135 cells: worse than always answering the commoner label. It beats baseline on 3 of 9 calls and is *anti-correlated* on `biz.website.dated` (31% against 69%). `biz.team_photos.missing` has precision **20%** — it answers "missing" almost every time. **Not shipped, not wired into collection.** **DoD:** scored per class against both trivial baselines, on an exam built before the skill existed. *(The pattern is the finding: the two calls backed by a real count — `bio_pages.missing` 77% vs 62%, `service_pages.missing` 77% vs 69% — are the two that win. Every call that loses is one a person makes **by looking at the page**: is it dated, are those real team photos, is the gallery clinical. The exam inputs are text, links and alt attributes, with no image, so this is most likely an input failure rather than a model one — but that is a hypothesis with an obvious test, not a conclusion, and the skill stays out of the product until it beats the baseline.)*
